...
class PhysicalActivityViewModel : ObservableObject {
    ...
    @Published var onePhysicalActivity : PhysicalActivity = PhysicalActivity(date: Date.now, duration: 0, caloriesBurned: 0, idUser: UUID(), idIntensity: UUID(), idActivityType: UUID())
    ...
    func fetchOneById(idPhysicalActivity: UUID) {
        guard let url = URL(string: baseUrl + idPhysicalActivity.uuidString) else {
            print("Invalid URL")
            return
        }
        var request = URLRequest(url: url)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data {
                do {
                    let decoder = JSONDecoder()
                    decoder.dateDecodingStrategy = .iso8601
                    let decodedPhysicalActivity = try decoder.decode(PhysicalActivity.self, from: data)
                    DispatchQueue.main.async {
                        self.onePhysicalActivity = decodedPhysicalActivity
                    }
                } catch {
                    print("Error decoding data : \(error)")
                }
